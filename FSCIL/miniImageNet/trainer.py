...
parser.add_argument('--N-shot', type=lambda s: [int(item.strip()) for item in s.split(',')],
                    default='10000000,5,5,5,5,5,5,5,5',
...
parser.add_argument('--move-to-gp-epoch', type=int, default=40, help='epoch to start training with GP')
...
exp_name = f'FSCIL_MiniImageNet_seed_{args.seed}_outputscale_{args.outputscale}_gibbs_outputscale_{args.gibbs_outputscale}' \
...
session_loader = MiniImagenetEpisodes(dataset_path, use_val=True)
...
def create_data_loaders():
    ...
    for i in range(1, num_sessions + 1):
        if i == 1:
            trainset, valset, testset = session_loader.get_episode_datasets(i)
            base_val_loader = DataLoader(valset, batch_size=test_batch_size, num_workers=args.num_workers)
        else:
            trainset, testset = session_loader.get_episode_datasets(i)

        # drop last needs to be False to accommodate for few shot sessions
        train_loaders.append(DataLoader(trainset, batch_size=batch_size, num_workers=args.num_workers,
                                        shuffle=True, drop_last=False))

        test_loaders.append(DataLoader(testset, batch_size=test_batch_size, num_workers=args.num_workers))
...
# build initial model
model = ModelBinaryTree(args, device, pretrained=False)

# print summary
summary(model.features, input_size=(3, 84, 84), device='cpu')
model.to(device)
